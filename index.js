...
db.serialize(() => {
    ...
    db.get("SELECT COUNT(*) as count FROM barbeiros WHERE cargo = 'Gerente'", (err, row) => {
        ...
        if (row.count === 0) {
            db.run(`
                INSERT INTO barbeiros (nome, cpf, email, telefone, cargo, senha) 
                VALUES ('Administrador do Sistema', '12345678900', 'admin@sistema.com', '(00) 00000-0000', 'Gerente', 'admin123')
            `, function(err) {
                if (err) {
                    console.error("Erro ao inserir admin padrão:", err);
                } else {
                    console.log("Usuário admin padrão criado: CPF 12345678900, Senha: admin123");
                }
            });
        }
    });
...
app.post("/barbeiros", (req, res) => {
    const { nome, cpf, email, telefone, especialidade, endereco, cargo, senha } = req.body;

    if (!nome || !cpf) {
        return res.status(400).send("Nome e CPF são obrigatórios.");
    }

    const query = `INSERT INTO barbeiros (nome, cpf, email, telefone, especialidade, endereco, cargo, senha) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`;
    db.run(
        query,
        [nome, cpf, email, telefone, especialidade, endereco, cargo || 'Barbeiro', senha || ''],
        function (err) {
            if (err) {
...
// Atualizar barbeiro por CPF
app.put("/barbeiros/cpf/:cpf", (req, res) => {
    const cpf = req.params.cpf;
    const { nome, email, telefone, especialidade, endereco, cargo, senha } = req.body;

    if (!nome) {
        return res.status(400).send("Nome é obrigatório.");
    }

    let query = `UPDATE barbeiros SET nome = ?, email = ?, telefone = ?, especialidade = ?, endereco = ?, cargo = ?`;
    let params = [nome, email, telefone, especialidade, endereco, cargo];
